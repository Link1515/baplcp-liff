<template>
  <div>
    <header>
      <h1
        class="flex h-28 flex-col items-center justify-center bg-blue-950 text-center text-3xl text-white"
      >
        <span class="text-3xl">華江高中</span>
        <small class="text-xl">112/01/01 ~ 112/03/31</small>
        <small class="text-xl">季打</small>
      </h1>
    </header>

    <div class="container py-8 pb-20">
      <h3 class="text-center text-xl">報名人數： 4/18</h3>
      <h3 class="text-center text-xl">費用： 300 NTD</h3>
      <h3 class="mb-4 text-center text-xl">打球次數： 4</h3>

      <ul>
        <li class="flex">
          <span class="mr-2">1.</span>
          <span class="mr-auto">佳穎</span>
          <span>報名成功</span>
        </li>
        <li class="flex">
          <span class="mr-2">2.</span>
          <span class="mr-auto">勇成</span>
          <span>報名成功</span>
        </li>
        <li class="flex">
          <span class="mr-2">3.</span>
          <span class="mr-auto">乃瑄</span>
          <span>繳費成功</span>
        </li>
      </ul>
    </div>

    <div
      class="fixed bottom-0 flex h-16 w-full items-center justify-center bg-slate-300 px-4"
    >
      <button class="rounded-full bg-green-500 px-4 py-1">立即報名</button>
    </div>
  </div>
</template>
